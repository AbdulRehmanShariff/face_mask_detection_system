# import os, cv2, numpy as np, base64, time
...
#     if pred < 0.5:
#         return "With Mask", round(float((1 - pred) * 100), 2), "#22c55e", "safe"
#     return "No Mask", round(float(pred * 100), 2), "#ef4444", "danger"

# def draw_and_process(image):
...
#         if face_img.size == 0:
#             continue
#         label, conf, hex_c, status = predict_mask(face_img)
#         hc = hex_c.lstrip("#")
...
#             if fi.size == 0:
#                 continue
#             lbl, conf, hx, st = predict_mask(fi)
#             if lbl == "With Mask":
#                 mc += 1
#             else:
#                 nc += 1
...
#         h, w = frame.shape[:2]
#         ov = frame.copy()
#         cv2.rectangle(ov, (0, 0), (w, 50), (10, 10, 20), -1)
#         cv2.addWeighted(ov, 0.75, frame, 0.25, 0, frame)
#         cv2.putText(frame, f"Faces: {len(faces)}", (15, 32),
#                     cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 215, 255), 2)
#         cv2.putText(frame, f"Mask: {mc}", (160, 32),
#                     cv2.FONT_HERSHEY_SIMPLEX, 0.75, (34, 197, 94), 2)
#         cv2.putText(frame, f"No Mask: {nc}", (310, 32),
#                     cv2.FONT_HERSHEY_SIMPLEX, 0.75, (60, 60, 239), 2)
#         cv2.putText(frame, "MaskGuard AI | 98.87%", (w - 270, 32),
...
#     spath = os.path.join(app.config["UPLOAD_FOLDER"], fname)
#     file.save(spath)
#     results, annotated = draw_and_process(cv2.imread(spath))
#     rpath = os.path.join(app.config["UPLOAD_FOLDER"], "result_" + fname)
#     cv2.imwrite(rpath, annotated)
#     with open(rpath, "rb") as f:
#         img64 = base64.b64encode(f.read()).decode()
#     if not results:
#         return jsonify({"error": "No face detected!", "image": img64, "results": []})
#     mc = sum(1 for r in results if r["label"] == "With Mask")
#     nc = sum(1 for r in results if r["label"] == "No Mask")
#     stats["total"] += len(results)
#     stats["mask"]  += mc
#     stats["nomask"] += nc
#     return jsonify({"image": img64, "results": results,
#                     "total_faces": len(results),
#                     "mask_count": mc, "nomask_count": nc})

# @app.route("/stats")
# def get_stats():
#     return jsonify({**stats, "uptime": int(time.time() - stats["start"])})
