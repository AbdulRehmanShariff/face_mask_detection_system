...
# ✅ FIXED: Safe generate_frames that won't crash on servers without webcam
def generate_frames():
    IS_SERVER = not os.path.exists("/dev/video0") and os.environ.get("RENDER")
    if IS_SERVER:
        # Return a placeholder image instead of crashing
        placeholder = np.zeros((480, 640, 3), dtype=np.uint8)
        cv2.putText(placeholder, "Webcam not available", (120, 220),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.0, (100, 100, 100), 2)
        cv2.putText(placeholder, "Use Upload tab instead", (130, 270),
        ...
        while True:
            yield (b"--frame\r\nContent-Type: image/jpeg\r\n\r\n"
                   + frame_bytes + b"\r\n")
            time.sleep(5)  # send same frame every 5 seconds, no CPU waste
        return
    ...
    if not cap.isOpened():
        placeholder = np.zeros((480, 640, 3), dtype=np.uint8)
        cv2.putText(placeholder, "Camera not found", (160, 240),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.0, (100, 100, 100), 2)
        _, buffer = cv2.imencode(".jpg", placeholder)
        frame_bytes = buffer.tobytes()
        while True:
